-- Rime Script >https://github.com/baopaau/rime-lua-collection/blob/master/snotest_translator.lua
-- 簡易計算器（執行任何Lua表達式）
--
-- 格式：=<exp>
-- Lambda語法糖：\<arg>.<exp>|
...
--   `snotest_translator = require("snotest_translator")`
-- - 在 <rime>/<schema>.schema.yaml 新增：
--   `engine/translators/@next: lua_translator@snotest_translator`
...
local function getIp()
  ...
  local body, code, headers, status = http.request("https://ifconfig.me/")

  if code == 200 then
    return body -- The body contains the response text which is your IP address
  else
    return status
  end
end
...
local greedy = true
local function snotest_translator(input, seg)
  ...
  -- yield(Candidate("number", seg.start, seg._end, exp, "ip表達1"))

  -- req = io.open(os.getenv('APPDATA').."\\Rime\\req.log", "a")
  -- req:write("-- End of the test.lua")
  -- req:close()

  -- res = io.open(os.getenv('APPDATA').."\\Rime\\res.log", "r")
  -- res:read("-- End of the test.lua res")
  -- res:close(res)

  -- local http = require("socket.http")
  -- yield(Candidate("number", seg.start, seg._end, exp.."1", "表達式3"))
  -- -- local body, code, headers, status = http.request("https://ifconfig.me/")
  -- yield(Candidate("number", seg.start, seg._end, exp.."2", "ip表達2"))
  -- local ip = ""
  -- if code == 200 then
  --   ip = body
  --   -- return body -- The body contains the response text which is your IP address
  ...
  --   -- return status
  -- end
  --   yield(Candidate("number", seg.start, seg._end, ip, "ip地址2"))
  -- yield(Candidate("number", seg.start, seg._end, exp.."2", "ip表達2"))
